...
for episode in range(start_episode, start_episode + 100000):
    ...
    action2 = 112
    if player == 1:
        state, reward2, _ = env.step(action2, 1)  # 先手落子
        player = 0

    state2 = state.clone()
    state2[[0, 1]] = state2[[1, 0]]
    while not done:
        # player1 落子
        state1 = state.clone()
        next_state = state1.clone()
        while player == 0:
            for event in pygame.event.get():
                ...
                if event.type == pygame.MOUSEBUTTONDOWN:
                    ...
                    row = y // 50
                    # 判断落子合法性
                    if next_state[0][row][col] == 0 and next_state[1][row][col] == 0:
                        action1 = row * 15 + col
                        next_state, reward1, done = env.step(action1, 0)
                        player = 1
                        if done:
                            reward2 = -1.0
        ...
        # player2 落子
        state2_last = state2.clone()
        state2 = next_state.clone()
        next_state = state2.clone()
        state2[[0, 1]] = state2[[1, 0]]
        # 将人类的落子存入经验池
        replay_buffer.push(state2_last, action2, reward2, state2, done)
        while player == 1:
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    pygame.quit()
                ...
                if event.type == pygame.MOUSEBUTTONDOWN:
                    ...
                    if next_state[0][row][col] == 0 and next_state[1][row][col] == 0:
                        action2 = row * 15 + col
                        next_state, reward2, done_opponent = env.step(action2, 1)
                        player = 0
        ...
        # 存入经验池
        replay_buffer.push(state1, action1, reward1, next_state_for_model, done)
    ...
    if episode % 5 == 0:
        try:
            logging.info(f"Episode {episode}, Total Reward: {total_reward:.2f}, Loss: {loss.item():.4f}")
        except:
            logging.info(f"Episode {episode}, Total Reward: {total_reward:.2f}, Loss: no loss")
    if episode % 20 == 0:
        torch.save({
            'episode': episode,
