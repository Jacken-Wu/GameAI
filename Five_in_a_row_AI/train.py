...
show_gui = True
logging.info(f"Show GUI: {show_gui}")
...
for episode in range(start_episode, start_episode + 100000):
    ...
    while not done:
        ...
        # 对下一个状态处理视角转换（供对手落子）
        next_state_for_opponent = next_state.clone()
        next_state_for_opponent[[0, 1]] = next_state_for_opponent[[1, 0]]

        # 对手落子
        next_action = select_action(next_state_for_opponent, 0.0)
        next_state, _, done_opponent = env.step(next_action, 1)
